...
class FingerPrintThread(QThread):
    ...
    signal_van_tay_dang_nhap_hop_le = pyqtSignal(int)
    signal_van_tay_that_bai = pyqtSignal(str, str)
    signal_id_vantay_hop_le = pyqtSignal(int)
    ...
    def run(self):
        ...
        while self.running:
            ...
            stateFind_FingerPrint, idDN = self.get_fingerprint_id()
            ...
            if stateFind_FingerPrint == True:
                ...
                # Chuyen trang chinh
                self.signal_id_vantay_hop_le.emit(idDN)
                self.trang_chinh()
    ...
    def get_fingerprint_id(self):
        try:
            ...
            state_find, idDN = vt.find_van_tay(ds_vanTay)
            return state_find, idDN
        except:
            pass
...
class MainWindow(QMainWindow):
    ...
    def vao_trang_dang_nhap(self):
        ...
        self.thread_van_tay.signal_id_vantay_hop_le.connect(self.id_vantay_dang_nhap_thanh_cong)
    ...
    def id_vantay_dang_nhap_thanh_cong(self, value_Signal_idVT):
        print("ID vân tay: ", value_Signal_idVT)
        dsTK = get_all_info_database(urlUser, 0)
        tkDNVt = dsTK.index(value_Signal_idVT)
        print("Tài khoản đăng nhập VT: ", tkDNVt)
    ...
    def trang_dang_ky(self):
        ...
        if self.tin_hieu_dang_ky == False:
            self.uic.btn_ql_dn.clicked.connect(self.trang_dang_nhap)
