...
class CollectFace(QThread):
    ...
    def run(self):
        global tt_Webcam, state_face, idNewFace
        tt_Webcam = "Bật"
        state_face = "collect"
        ...
        while self._running and tt_Webcam == "Bật":
            if cap.isOpened():
                ...
                if ret:
                    ...
                    if count_img >= 200:  # ESC để thoát hoặc thu thập đủ 200 ảnh
                        break
                else:
                    print("Ket noi lai camera......")
                    cap = cv2.VideoCapture("/dev/video0")
        ...
        print("đã tắt cam")
        with open("/home/lapdo/NCKH/FaceID/user.txt", "a") as f:
            f.writelines(f"{self.newIdFace}\n")
        idNewFace = self.newIdFace
        # Train face
        path_folder_img = "/home/lapdo/NCKH/FaceID/data_face"
        state_train = self.trainFace(path_folder_img)
        return
...
class MainWindow(QMainWindow):
    ...
    def dang_ki_van_tay(self):
        global state_face, tt_Webcam, vt, state_train
                ...
                if state_tk_van_tay_dki == True:
                    ...
                    if stateAdd == False:
                        ...
                        if return_choose == QMessageBox.StandardButton.Yes:
                            ...
                            while state_train == False: # Đã train xong
                                
                                self.uic.btn_xn_dk.setDisabled(False)
                                self.uic.btn_ql_dn.setDisabled(False)
                                self.uic.btn_vt_dki.setDisabled(False)
                                listNewInfoUser = [self.tkDki, self.mkDki, self.idNewVanTay, ] # tk mk vantay face
                                firebase.put(urlUser, self.idNewUsers, listNewInfoUser)
                                
                            else: # Đang train
                                self.uic.btn_xn_dk.setDisabled(True)
                                self.uic.btn_ql_dn.setDisabled(True)
                                self.uic.btn_vt_dki.setDisabled(True)
